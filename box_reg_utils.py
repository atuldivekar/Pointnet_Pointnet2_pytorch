...
def estimate_gt_box(reg_bbox_preds, prop_bbox, is_gpu):   #if is_gpu: all i/p and o/p on gpu, else cpu
        ...
        gt_box_est[:,0] =  reg_bbox_preds[:,0] * prop_bbox[:,4] + prop_bbox[:,0]
        gt_box_est[:,1] =  reg_bbox_preds[:,1] * prop_bbox[:,4] + prop_bbox[:,1]
...
class RotatedRect:
    ...
    def get_contour(self):
        ...
        rc = shapely.affinity.rotate(c, self.angle, use_radians=True,origin = 'centroid')   #+ve ccw 0 to 2pi
...
def IoU_3D(gtbox,gtbox_pred):  #shapely needs i/ps in cpu, so always in cpu  #assumes boxes rotated only around vertical 
    ...
    gt_velo_fwd = gtbox[:,3]  #dx 
    gt_velo_across = gtbox[:,4]  #dy
    ...
    gtp_velo_fwd = gtbox_pred[:,3]       #dx
    gtp_velo_across = gtbox_pred[:,4]    #dy
    ...
    print(intersec_ht)
    '''
    gt_vol  = gt_velo_fwd  * gt_velo_across * gt_velo_dim_vert
    gtp_vol = gtp_velo_fwd * gtp_velo_across * gtp_velo_dim_vert
    '''
    print('gt_vol')
    print(gt_vol)
    print('gtp_vol')
    ...
    for ind in range(B):
        ...
        #print(gt_velo_across[ind])
        #print(gt_velo_fwd[ind])
        #print(gt_velo_dim_vert[ind])
        #print(gt_rz)
        #print("\n")

        r1 = RotatedRect(gt_velo_mid_across[ind],gt_velo_mid_fwd[ind],gt_velo_across[ind],gt_velo_fwd[ind],gt_rz) #shapely needs all in cpu
        ...
        #print(gtp_velo_across[ind])
        #print(gtp_velo_fwd[ind])
        #print(gtp_velo_dim_vert[ind])
        #print(gtp_rz)
        #print("\n")

        r2 = RotatedRect(gtp_velo_mid_across[ind],gtp_velo_mid_fwd[ind],gtp_velo_across[ind],gtp_velo_fwd[ind],gtp_rz)
